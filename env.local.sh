...
if [ -f "$LOCALWP_SCRIPT_PATH" ]; then
    ...
    # Remove the last 3 lines from the script and store it in a variable
    MODIFIED_SCRIPT=$(sed '$d' "$LOCALWP_SCRIPT_PATH" | sed '$d' | sed '$d')
    ...
    eval "$MODIFIED_SCRIPT"

    echo "LocalWP environment loaded with the last 3 lines removed."
else
