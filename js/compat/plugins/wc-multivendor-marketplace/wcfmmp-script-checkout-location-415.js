jQuery( function( $ ) {
	...
	function initializeCheckoutLocation() {
		...
		var $wcfmmp_user_location_lat = jQuery( "#wcfmmp_user_location_lat" ).val();
		var $wcfmmp_user_location_lng = jQuery( "#wcfmmp_user_location_lng" ).val();

		function initialize() {
			if ( typeof wcfm_maps === 'undefined' || typeof wcfmmp_checkout_map_options === 'undefined' ) { return; }

			if ( wcfm_maps.lib == 'google' ) {
				var $locationField = $( '#wcfmmp_user_location' );
				if ( ! $locationField.length || typeof google === 'undefined' || ! google.maps ) { return; }

				$locationField.parent().find( '.wcfmmmp_locate_icon' ).remove();
				$locationField.parent().append( '<i class="wcfmmmp_locate_icon" style="background-image: url(' + wcfmmp_checkout_map_options.locate_svg + ')"></i>' );
				...
				if ( $( '#leaflet_wcfmmp_user_location' ).length === 0 ) {
		...
		if ( jQuery( "#wcfmmp_user_location_lat" ).length > 0 ) {
			setTimeout( function() {
				...
				if ( navigator.geolocation ) {
					$( document.body ).off( 'click.fcWcfmmpLocate', '.wcfmmmp_locate_icon' ).on( 'click.fcWcfmmpLocate', '.wcfmmmp_locate_icon', function () {
