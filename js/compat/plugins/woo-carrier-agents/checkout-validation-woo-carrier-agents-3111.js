...
(function (root, factory) {
	...
	var isValidateField = function( field, formRow, validationEvent ) {
		// Bail if not a woo carrier agents method field
	...
	/**
	 * Validate if a Woo Carrier Agent shipping method collection point is selected.
	 * @param  {Field}    field            Field for validation.
	 * @param  {Element}  formRow          Form row element.
	 * @param  {String}   validationEvent  Event that triggered the validation.
	 * @return {Boolean}                   Whether a Woo Carrier Agent shipping method collection point has been selected.
	 */
	var validateField = function( field, formRow, validationEvent ) {
		// Bail if Woo Carrier Agent shipping method field is empty
		if ( '' === field.value ) {
			// Scroll to section
